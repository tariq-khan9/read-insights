...
import { GET_ALL_POSTS_BY_AUTHOR, GET_COMMENTS } from '../../services/graphql/queryMutations'
...
const IndexDash = () => {
   ...
   const { data, loading, error } = useQuery(GET_ALL_POSTS_BY_AUTHOR, {
  ...
  if(data.posts.length==0) return <h1 className='flex h-screen bg-transparent justify-center pt-60 font-barlow text-sky-500 text-[40px]'>You havnt any post!</h1>

  return (
    <div className='px-20 lg:px-60  flex flex-row justify-center relative'>

      <div className='w-[60%] space-y-12 pb-8'>
        {
          data.posts.map((post)=>(
            <PostCard post={post} />
          ))
